...
export const Home = () => {
  ...
  const [activityFeed, setActivityFeed] = useState([
    { id: 1, user: 'Sarah K.', action: 'claimed Netflix deal', time: '2m ago', avatar: 'bg-blue-500' },
    { id: 2, user: 'Mark T.', action: 'shared Spotify offer', time: '5m ago', avatar: 'bg-green-500' },
    { id: 3, user: 'Emma L.', action: 'saved $500 on gym', time: '8m ago', avatar: 'bg-purple-500' },
  ]);
  ...
  useEffect(() => {
    ...
    const activityInterval = setInterval(() => {
      const activities = [
        { user: 'Alex R.', action: 'joined premium club', avatar: 'bg-red-500' },
        { user: 'Lisa M.', action: 'found rare deal', avatar: 'bg-yellow-500' },
        { user: 'David P.', action: 'earned $200 cashback', avatar: 'bg-indigo-500' },
        { user: 'Maya S.', action: 'unlocked VIP status', avatar: 'bg-pink-500' },
      ];
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              >
                <Button size="lg" className="gap-2 bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90" asChild>
                  <Link to="/login">
              ...
              >
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <motion.div
                      key={i}
                      className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-accent border-2 border-background flex items-center justify-center text-xs font-semibold text-white"
              ...
              <div className="relative mx-auto w-80 h-[650px]">
                ...
                >
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-slate-50 to-white rounded-[2.5rem] overflow-hidden relative">
                    {/* Premium Header */}
                    <div className="h-28 bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 flex items-center justify-center relative overflow-hidden">
                      <motion.div 
                    ...
                    <div className="p-4 space-y-3 flex-1">
                      {[
                        { title: 'Goldman Sachs Access', value: '$50K', category: 'Financial', icon: DollarSign, gradient: 'from-yellow-400 to-orange-500' },
                        { title: 'Private Jet Shares', value: '€25K', category: 'Travel', icon: Globe, gradient: 'from-blue-500 to-cyan-400' },
                        { title: 'Exclusive Wine Club', value: '$15K', category: 'Luxury', icon: Award, gradient: 'from-purple-500 to-pink-500' }
                      ].map((deal, i) => (
                        <motion.div
                          key={deal.title}
                          initial={{ opacity: 0, y: 20 }}
                ...
                <motion.div 
                  className="absolute top-12 right-8 flex items-center gap-1 bg-green-500 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg"
                  initial={{ opacity: 0, y: -20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 2, duration: 0.5 }}
                >
                  <Users className="w-3 h-3" />
                  <CountUp end={10847} duration={2} />
                </motion.div>
              </div>
            </motion.div>
          </div>
